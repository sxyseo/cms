...
//    	if(!me.grid){
        	...
        	me.grid =Ext.create('Ext.grid.Panel', {
        	    ...
        	    columns: [{
    				 ...
    				 width:60,
    				 items: [{
    					iconCls: 'cms-data-manager-action-table-column',
     	                tooltip: '编辑列',
...
//     	                	//获得对话框
     	                   ...
     	                   editorWindow.title='['+rec.get('name')+']模型项目';
     	                    var tableColumnGrid=Ext.getCmp('cms-data-manager-table-column-grid');
 	                ...
 	                handler: function(grid, rowIndex, colIndex) {
                     	...
                     	//获得表单在的panel
 	                	var detailPanel = editorWindow.getComponent('cms-data-manager-add-panel');
 	                	//获得选中的数据
 	                    var rec = grid.getStore().getAt(rowIndex);
 	                    //获得表单，并把数据加载到表单内
        	    ...
        	    dockedItems: [{
                    ...
                    items: [{
                        ...
                        text: '删除',
                        disabled: true,
                        itemId:'cms-data-manager-btn-delete',
                        scope: this,
                        handler:function(){
                    ...
                    'selectionchange': function(view, records) {
                    	if(records.length){
                    		...
                    		if(v.data.systemDefined){
                    			me.grid.down('#cms-data-manager-btn-delete').setDisabled(true);
                    			return;
                    		}
                    	}
                    	me.grid.down('#cms-data-manager-btn-delete').setDisabled(!records.length);
                    },
                    //行双击事件
...
//    	}
    ...
    createEditorWindow:function(){
        ...
        var win = desktop.getWindow('cms-data-manager-editor-win');
        if (!win) {
        	 var form =Ext.create('Ext.form.Panel', {
                 url: 'cms/tableDefined/add',
                 id:'cms-data-manager-add-panel',
                 layout: 'anchor',
                 defaults: {
                     anchor: '100%'
                 },
            ...
            win = desktop.createWindow({
                id: 'cms-data-manager-editor-win',
                title: '详细信息',
                width:440,
                height:300,
                modal:true,
    ...
    createTableColumnWindow:function(){
		var me=this;
		var desktop = me.app.getDesktop();
		var win = desktop.getWindow('cms-data-manager-table-column-win');
		if (!win) {
			var grid=Ext.create('Ext.grid.Panel', {
				id:'cms-data-manager-table-column-grid',
				...
				columns:[{
					...
					items: [{
						...
						handler: function(grid, rowIndex, colIndex) {
                        	...
                        	//获得表单在的panel
    	                	var detailPanel = editorWindow.getComponent('cms-data-manager-table-column-editor-panel');
    	                	//获得选中的数据
    	                    var rec = grid.getStore().getAt(rowIndex);
    	                    //获得表单，并把数据加载到表单内
    	                    var form=detailPanel.getForm();
						...
						handler : function(grid,rowIndex,colIndex){
							...
							com.calm.platform.Utils.requestAjax('cms/tableColumn/order',{tableId: rec.get('tableId'),id: rec.get('columnName'),up:true},function(){
                				var tableColWin=Ext.getCmp('cms-data-manager-table-column-win');
...
//    	                				grid.getView().refresh();
				...
				dockedItems: [{
					...
					items: [{
						...
						handler: function(){
							...
							var tableColWin=Ext.getCmp('cms-data-manager-table-column-win');
							var formPanel=Ext.getCmp('cms-data-manager-table-column-editor-panel');
			...
			win = desktop.createWindow({
				id: 'cms-data-manager-table-column-win',
				title: '模型项目',
				width:830,
				height:500,
				modal:true,
    ...
    createTableColumnEditorWindow:function(){
    	var me=this;
    	var desktop = me.app.getDesktop();
        var win = desktop.getWindow('cms-data-manager-table-column-editor-win');
        if (!win) {
        	var form =Ext.create('Ext.form.Panel', {
        		url: 'cms/tableColumn/add',
                id:'cms-data-manager-table-column-editor-panel',
                layout: 'anchor',
                defaults: {
                    anchor: '100%'
                },
					...
					items:[{
					    ...
					    listeners:{
					    	change:function(combo,  newValue, oldValue, eOpts){
					    		var editorWindow=Ext.getCmp('cms-data-manager-table-column-editor-win');
					    		var detailPanel = editorWindow.getComponent('cms-data-manager-table-column-editor-panel');
					    		var form=detailPanel.getForm();
						...
						if (form.isValid()) {
							form.submit({
								...
								success: function(form, action) {
									Ext.Msg.alert('系统信息', action.result.message);
									var tableColWin=Ext.getCmp('cms-data-manager-table-column-win');
									var tableColGrid=Ext.getCmp('cms-data-manager-table-column-grid');
//									
            ...
            win = desktop.createWindow({
                id: 'cms-data-manager-table-column-editor-win',
                title: '模型项目详细信息',
                width:440,
                height:300,
                modal:true,
