...
public class ColumnDefinedAction extends
	...
	public String add(Model model) {
		List<FieldType> listAll = fieldTypeService.listAll();
		model.addAttribute("types", listAll);
		return super.add(model);
	}

	@RequestMapping(value = "/edit/", method = RequestMethod.POST)
	public String add(ColumnDefined entity, Model model) {
	...
	public String update(@PathVariable("id") Integer id, ColumnDefined entity,
			Model model) {
		if (StringUtils.isEmpty(entity.getName())) {
			addFieldError("name", "GLOBAL_E_00001");
		}
